...
#define NUM_VM 8 // no of virtual machines
#define NUM_RT 3 // resource types .. IMP -> keep this constant for now
#define MAX_ITER 1
#define FEASIBLE_FLAG 0 // 0 to continue even if an algo fails to give solution AND 1 to run only feasible problems. NOTE : 0 will not run/continue if OPTIMAL LP doesnt give a solution
#define CAPACITY_TYPE 0 // 0 is constant capacity for each case AND 1 is Random Capacityfor every case.
#define RANDOM_TYPE 1 // 1 is for Uniform Requirement AND 2 is for Rand time seed for Requirements. 
#define MACHINE_TYPE 0 // 0 is for Static defined machines and 1 for random Machines. 
#define MAPPING_FLAG 1 // 0 to turn off mapping output, 1 otherwise
...
static int
	populatebynonzero (CPXENVptr env, CPXLPptr lp, int ***R, int ***C,  int ***together, int *n_together, int **together_size, int ***distinct, int *n_distinct, int **distinct_size, int *vm_distinct, int NUM_PM);

static void
	free_and_null     (char **ptr),
	usage             (char *progname);

void initializeVariables(int ***R, int ***C, int **RC, int **TC, int **TR, int **minRC, int ***together, int *n_together, int **together_size, int ***distinct, int *n_distinct, int **distinct_size, int NUM_PM) {
...
void generateRandomCase(int ***R, int ***C, int **RC, int **TC, int **TR, int **minRC, int ***together, int *n_together, int **together_size, int ***distinct, int *n_distinct, int **distinct_size, int *vm_distinct, int iter, int randomType, int capacityType, int machineType, int NUM_PM) {
	...
	*together_size = (int *)malloc(*n_together * sizeof(int ));
	...
	for(i=0; i< *n_together; i++) {
		for(j=0; j<(*together_size)[i]; j++) {
			printf("Set Together %d and VM %d == %d\n",i, j, (*together)[i][j]);			
		}
	}
	...
	//	*n_together = rand() % (int)(NUM_VM/8);
	*n_distinct = rand() % 3;
	if( *n_distinct == 0 ) *n_distinct = 1;
	*n_distinct = 1;
	*distinct = (int **)malloc(*n_distinct * sizeof(int *));
	*distinct_size = (int *)malloc(*n_distinct * sizeof(int ));
	printf("Sets Distinct%d\n",*n_distinct);	
	*vm_distinct = 0;
	for(i=0; i< *n_distinct; i++) {

		//		temp_size = rand() % (NUM_VM/ *n_distinct);
		//temp_size = 3;
		//		(*distinct_size)[i] = temp_size;

		//		(*distinct)[i] = (int *)malloc(temp_size * sizeof(int));
		(*distinct)[i] = (int *)malloc(sizeof(int));
		(*distinct_size)[i] = 1;
		do {
			(*distinct)[i][0] = rand() % (NUM_VM-1);
		}while( tempVM[ (*distinct)[i][0] ] == 1 );

		*vm_distinct = *vm_distinct+1;
		tempVM[ (*distinct)[i][0] ] = 1;
	}

	for(j=0; j< 1; j++) {
		//	for(j=0; j< (int)(NUM_VM/5) - *n_distinct; j++) {

		if( *n_distinct == 1 ) setNo = 0;
		else setNo = rand() % (*n_distinct-1);
		(*distinct)[setNo] = (int *)realloc((*distinct)[setNo], ((*distinct_size)[setNo] + 1) * sizeof(int));

		do {
			(*distinct)[setNo][(*distinct_size)[setNo]] = rand() % (NUM_VM-1);
		}while( tempVM[ (*distinct)[setNo][(*distinct_size)[setNo]] ] == 1 );

		tempVM[ (*distinct)[setNo][(*distinct_size)[setNo]] ] = 1;
		*vm_distinct = *vm_distinct+1;
		(*distinct_size)[setNo]++;
	}
	//printf("vm distinct after sets generation %d\n", *vm_distinct);
	for(i=0; i< *n_distinct; i++) {
		for(j=0; j<(*distinct_size)[i]; j++) {
			printf("Set Distinct %d and VM %d == %d\n",i, j, (*distinct)[i][j]);			
		}
	}
...
int generateFirstFit(int ***R, int ***C, int ***together, int *n_together, int **together_size, int ***distinct, int *n_distinct, int **distinct_size, int NUM_PM) {

	int **remainPMCap = NULL;
	int usedPM[NUM_PM], i, j, k, l, m, assignedVM[NUM_VM], distinct_pm = 0;
	int num_pmused = 0, compFound = 0, distinct_flag = -1;
	...
	for( i=0; i < NUM_VM; i++) {
		compFound = 0;
		if( assignedVM[i] != -1 ) continue;
		for( j=0; j < *n_together; j++ ) {
			for( k=0; k < (*together_size)[j] ; k++ ) {
				if( (*together)[j][k] == i && assignedVM[i] == -1 ) {
					for( l=0; l<NUM_PM; l++ ) {
						...
						if( tempCompatibility ) {
							...
							for( m=0; m<(*together_size)[j]; m++ ) {
								assignedVM[(*together)[j][m]] = l;	
								if( MAPPING_FLAG ) printf("VM %d assigned to %d\n", (*together)[j][m], l);
							}
		...
		// check to see if Vm needs to be distinct
		for( j=0; j < *n_distinct; j++ ) {
			for( k=0; k < (*distinct_size)[j] ; k++ ) {
				if( (*distinct)[j][k] == i && assignedVM[i] == -1 && (*distinct_size)[j] > 1 ) {
					distinct_flag = j;
					break;
				}
			}
			if( distinct_flag != -1 ) break;
		}
		...
		for( j=0; j < NUM_PM; j++ ) {
			...
			distinct_pm = 0;
			if( distinct_flag != -1 ) {
				for( l=0; l< (*distinct_size)[distinct_flag]; l++ ) {
					if( assignedVM[l] == j ) {
						distinct_pm = 1;
					}
				}
			...
			if( tempCompatibility && distinct_pm == 0) {
				//				cout << "VM " << i << " Mapped to PM " << j << endl; 
				if( MAPPING_FLAG )  printf("VM %d assigned to PM %d\n",i,j);
...
int generateNextFit(int ***R, int ***C, int ***together, int *n_together, int **together_size, int ***distinct, int *n_distinct, int **distinct_size, int NUM_PM) {

	int **remainPMCap = NULL;
	int usedPM[NUM_PM], i, j, k, l, m, assignedVM[NUM_VM], distinct_flag = -1;
	int num_pmused = 0, compFound = 0, lastPMUsed = 0, distinct_pm = 0;
...
int generateBestFit(int ***R, int ***C, int **RC, int **TC, int **TR, int **minRC, int iter, int ***together, int *n_together, int **together_size, int ***distinct, int *n_distinct, int **distinct_size, int NUM_PM) {

	int **remainPMCap = NULL;
	int usedPM[NUM_PM], i, j, k, l, m, bestPM = NULL, compatiblePM[NUM_PM], assignedVM[NUM_VM], distinct_flag = -1, distinct_pm = 0;
			...
			for( k=0; k < (*together_size)[j] ; k++ ) {
				if( (*together)[j][k] == i && assignedVM[i] == -1 ) {
					...
					if( !compFound || bestPM == -1 ) {
						...
						for( m=0; m<(*together_size)[j]; m++ ) {
							if( MAPPING_FLAG ) printf("VM %d assigned to PM %d\n",i,bestPM);
							assignedVM[(*together)[j][m]] = bestPM;	
						}
						usedPM[bestPM]++;
					}
				}
...
int generateWorstFit(int ***R, int ***C, int **RC, int **TC, int **TR, int **minRC, int iter, int ***together, int *n_together, int **together_size, int ***distinct, int *n_distinct, int **distinct_size, int NUM_PM) {
...
int generateRandomFit(int ***R, int ***C, int **RC, int **TC, int **TR, int **minRC, int iter, int ***together, int *n_together, int **together_size, int ***distinct, int *n_distinct, int **distinct_size, int NUM_PM) {

	int **remainPMCap = NULL;
	int usedPM[NUM_PM], i, j, k, l, m, bestPM = -1, compatiblePM[NUM_PM], assignedVM[NUM_VM];
	int num_pmused = 0, randPM, compFound = 0, compPMListlen = 0, ctr = 0, distinct_flag = -1, distinct_pm = 0;
			...
			for( k=0; k < (*together_size)[j] ; k++ ) {
				if( (*together)[j][k] == i && assignedVM[i] == -1 ) {
					...
					if( !compFound ) {
						...
						for( m=0; m<(*together_size)[j]; m++ ) {
							if( MAPPING_FLAG )	printf("VM %d assigned to PM %d\n", (*together)[j][m], randPM);
							assignedVM[(*together)[j][m]] = randPM;	
						}
	...
	main (int argc, char **argv)
{
	...
	*/
	...
	int *TR = NULL;
	int *minRC = NULL;
	int **together = NULL, **distinct = NULL;
	int NUM_PM = 0, n;
	int n_together, *together_size = NULL, n_distinct, *distinct_size = NULL, vm_distinct;
	...
	for( n=0; n<3; n++ ) {
		...
		NUM_PM = 8; // Direct Assigning for debugging 
		printf("Number of PM for n %d is %d\n", n, NUM_PM);

		initializeVariables(&R, &C, &RC, &TC, &TR, &minRC, &together, &n_together, &together_size, &distinct, &n_distinct, &distinct_size, NUM_PM);

		//	initializeVariables(&R, &C, &RC, &TC, &TR, &minRC);
		if( CAPACITY_TYPE == 0 )
			generateRandomCase(&R, &C, &RC, &TC, &TR, &minRC, &together, &n_together, &together_size, &distinct, &n_distinct, &distinct_size, &vm_distinct, 1, 1, 1, 1, NUM_PM);
		...
		*/
		...
		for( iter=0; iter<=MAX_ITER; iter++) {
			...
			if( CAPACITY_TYPE == 0)
				generateRandomCase(&R, &C, &RC, &TC, &TR, &minRC, &together, &n_together, &together_size, &distinct, &n_distinct, &distinct_size, &vm_distinct, iter, 1, 0, MACHINE_TYPE, NUM_PM);
			else 
				generateRandomCase(&R, &C, &RC, &TC, &TR, &minRC, &together, &n_together, &together_size, &distinct, &n_distinct, &distinct_size, &vm_distinct, iter, RANDOM_TYPE, 1, MACHINE_TYPE, NUM_PM);
			//printf("After Random Case Gen\n");
			fprintf(fp, "%d ", iter);
			//printf("Before Heuristic Algo\n");
			nPMUsed_out = generateFirstFit(&R, &C, &together, &n_together, &together_size, &distinct, &n_distinct, &distinct_size, NUM_PM);
			...
			nPMUsed_out = generateNextFit(&R, &C, &together, &n_together, &together_size, &distinct, &n_distinct, &distinct_size, NUM_PM);
			fprintf(fp, ", %d ", nPMUsed_out);
			...
			nPMUsed_out = generateBestFit(&R, &C, &RC, &TC, &TR, &minRC, iter, &together, &n_together, &together_size, &distinct, &n_distinct, &distinct_size, NUM_PM);
			fprintf(fp, ", %d ", nPMUsed_out);
			...
			nPMUsed_out = generateWorstFit(&R, &C, &RC, &TC, &TR, &minRC, iter, &together, &n_together, &together_size, &distinct, &n_distinct, &distinct_size, NUM_PM);
			fprintf(fp, ", %d ", nPMUsed_out);
			...
			nPMUsed_out = generateRandomFit(&R, &C, &RC, &TC, &TR, &minRC, iter, &together, &n_together, &together_size, &distinct, &n_distinct, &distinct_size, NUM_PM);
			fprintf(fp, ", %d ", nPMUsed_out);
			...
			*/
			//printf("before Populating %d\n");
			status = populatebynonzero (env, lp, &R, &C, &together, &n_together, &together_size, &distinct, &n_distinct, &distinct_size, &vm_distinct, NUM_PM);
	...
	populatebynonzero (CPXENVptr env, CPXLPptr lp, int ***R, int ***C, int ***together, int *n_together, int **together_size, int ***distinct, int *n_distinct, int **distinct_size, int *vm_distinct, int NUM_PM)
{
	...
	const int NUMROWS = (int)NUM_VM + NUM_PM * NUM_RT + 1 + NUM_PM * (*n_distinct); ///+1 is for Vicinity Constraint // without the indicator rows
	const int NUMNZ = (int)(NUM_VM * NUM_PM * NUM_RT) + (NUM_VM * NUM_PM) + NUM_PM + ((*n_distinct) * NUM_PM * *vm_distinct); // nonzeros without the indicator ones. // the last term is not exact but the maximum number
	int      status    = 0;
	...
	for( j=0; j<*n_distinct; j++ ) {
		for( k=0; k<NUM_PM; k++ ) {
			rowname[tempRowConst] = malloc(100);
			sprintf(rowname[tempRowConst], "c6_%d", tempRowConst-NUM_VM-NUM_PM * NUM_RT - 1);
			sense[tempRowConst]   = 'L';
			rhs[tempRowConst]     = 1; 
			tempRowConst++;
		}
	...
	for (j = 0; j < NUMCOLS + NUM_PM + NUM_PM; j++) {
		if( j < NUMCOLS ) {
			...
			//			colname[j][1] = (char)j-NUMCOLS+1;
			//			itoa(j-NUMCOLS+1, colname[j][1], 10);
			coltype[j] = 'B'; // all binary variables Vj
		}
	}
	...
	tempRowIndex++;
	//printf("Row Index after Together set %d\n", tempRowIndex);
	for( j=0; j<*n_distinct; j++ ) {
		for( k=0; k<NUM_PM; k++ ) {
			for( i=0; i<(*distinct_size)[j]; i++ ) {
				rowlist[tempArrayIndex] = tempRowIndex;
				//printf("adding col %d = %d\n", tempArrayIndex, i*NUM_PM+j);
				collist[tempArrayIndex] = (*distinct)[j][i] * NUM_PM + k; 
				vallist[tempArrayIndex] = 1;
				tempArrayIndex++;
			}
			tempRowIndex++;
